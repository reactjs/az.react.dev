...
[`createRoot()`](/docs/react-dom-client.html#createRoot) funksiyasının elastikliyi React-i digər applikasiyalara əlavə etməyə imkan yaradır.
...
Aşağıda `ParagraphView` Backbone görünüşü yaradacağıq. Bu görünüş, Backbone-un `render()` funksiyasını əvəzləyərək `<Paragraph>` React komponentini Backbone tərəfindən təmin edilən DOM elementinə (`this.el`) render edəcək. Biz burada da [`ReactDOM.createRoot()`-dən](/docs/react-dom-client.html#createroot) istifadə edəcəyik:
